...
aivox is designed to scale horizontally, so its inputs arrive as queue messages rather than
synchronous calls. Like jesoos it runs as a trusted system user and skips Row-Level Security for
performance.
